...
def add_step_to_emr(cluster_id, step_name, jar, args):
    ...
    try:
            ...
            Steps=[
                {
                    ...
                    "HadoopJarStep": {"Jar": jar, "Args": args},
                }
            ],
        )
        ...
        logger.info(
            f"Added step '{step_name}' (ID: {step_id}) to EMR cluster {cluster_id}"
        )
        return step_id
    except Exception as e:
        logger.error(
            f"Error adding step '{step_name}' to EMR cluster {cluster_id}: {e}"
        )
        return None
    ...
    while True:
        ...
        if state == "COMPLETED":
            ...
            if "FailureDetails" in response["Step"]["Status"]:
                reason = response["Step"]["Status"]["FailureDetails"].get(
                    "Reason", "Unknown"
                )
...
def main():
    ...
    logger.info(
        f"Running Spark jobs on EMR cluster {cluster_id} in {environment} environment"
    )

    # Run the vehicle and location performance metrics job
    logger.info("Running vehicle and location performance metrics job")
    if not run_vehicle_location_metrics_job(
        cluster_id, S3_CONFIG["bucket_name"], environment
    ):
    ...
    if not run_user_transaction_analysis_job(
        cluster_id, S3_CONFIG["bucket_name"], environment
    ):
