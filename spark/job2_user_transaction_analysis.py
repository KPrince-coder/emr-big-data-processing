...
from pyspark.sql.functions import (
    ...
    datediff, hour, lit, when, round, date_format, to_date
)
...
def load_data(spark, s3_bucket, raw_data_prefix):
    ...
    users_df = (spark.read.option("header", "true")
               .option("inferSchema", "true")
               .csv(f"s3://{s3_bucket}/{raw_data_prefix}users/"))
    
    return transactions_df, users_df

def analyze_daily_transactions(transactions_df):
    """
    ...
    transactions_with_date = transactions_df.withColumn(
        "rental_date", 
        to_date(col("rental_start_time"))
    )
    
    # Calculate metrics by day
    daily_metrics = (transactions_with_date
        .groupBy("rental_date")
        .agg(
...
def analyze_transaction_patterns(transactions_df):
    ...
    transactions_with_hour = transactions_df.withColumn(
        "hour_of_day", 
        date_format(col("rental_start_time"), "HH").cast("int")
    )
    
    # Calculate metrics by hour of day
    hourly_metrics = (transactions_with_hour
        .groupBy("hour_of_day")
        .agg(
            count("rental_id").alias("total_transactions"),
            sum("total_amount").alias("total_revenue"),
    ...
    transactions_with_day = transactions_df.withColumn(
        "day_of_week", 
        date_format(col("rental_start_time"), "u").cast("int")
    )
    
    # Calculate metrics by day of week
    day_of_week_metrics = (transactions_with_day
        .groupBy("day_of_week")
        .agg(
            count("rental_id").alias("total_transactions"),
            sum("total_amount").alias("total_revenue"),
...
def main():
    ...
    try:
        ...
        # Analyze transaction patterns
        hourly_metrics, day_of_week_metrics = analyze_transaction_patterns(transactions_df)
        
        # Write the results to S3 in Parquet format
        daily_metrics.write.mode("overwrite").parquet(
            f"s3://{s3_bucket}/{processed_data_prefix}user_transaction_analysis/daily_metrics/"
        )
        
        user_metrics.write.mode("overwrite").parquet(
            f"s3://{s3_bucket}/{processed_data_prefix}user_transaction_analysis/user_metrics/"
        )
        
        hourly_metrics.write.mode("overwrite").parquet(
            f"s3://{s3_bucket}/{processed_data_prefix}user_transaction_analysis/hourly_metrics/"
        )
        
        day_of_week_metrics.write.mode("overwrite").parquet(
            f"s3://{s3_bucket}/{processed_data_prefix}user_transaction_analysis/day_of_week_metrics/"
        )
